...
ROUND(COUNT(*) * 100/ (SELECT CAST(COUNT(*) AS float) FROM dbo.HRDataset), 2) AS active_percentage
FROM dbo.HRDataset
WHERE EmploymentStatus = 'Active';
...
-- What is the number of Active Employees based on Age Group and Gender:
WITH t0 AS (
SELECT 
Sex,
CASE WHEN Age <= 24 THEN'Under 24'
...
ROUND((CAST(separations AS float)/(SELECT COUNT(*) FROM dbo.HRDataset WHERE EmploymentStatus <> 'Active' )),2) * 100 AS attritionRateoftotalAttrition
FROM (SELECT 
Department,
COUNT(*) AS total_emps,
